...
for (let i = 1; ; i++) {
    ...
    // check if ALL octopi flashed on this step
    // if that is the case, stop the simulation
    let didAllFlash = map.every(row => row.every(octopus => octopus.didFlashThisStep));
    if (didAllFlash) {
        console.log(`on step ${i}, all octopi flashed!`.green);
        break;
    }
}
