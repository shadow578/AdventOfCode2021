...
// print to image file using jimp
paper.printImage = function (imgPath) {
    // create image with jimp
    // add a 2px border around the image for better readability
    let img = new Jimp(this.width + 4, this.height + 4, Jimp.rgbaToInt(0, 0, 0, 255));

    // iterate all dots on the map, and color them on the image
    for (let y = 0; y < this.height; y++) {
        for (let x = 0; x < this.width; x++) {
            let c = (this[y][x] == '#') ? Jimp.rgbaToInt(255, 0, 0, 255) : Jimp.rgbaToInt(0, 0, 0, 255);
            img.setPixelColor(c, x + 2, y + 2);
        }
    }
...
// there's no way we can read it on the console, OR parse...
// so we print to output.png and read it by hand
paper.printImage("./exclude/output.png");
